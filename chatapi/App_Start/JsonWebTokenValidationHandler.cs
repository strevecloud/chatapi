    ...
    public class JsonWebTokenValidationHandler : DelegatingHandler
    {
        ...
        protected override Task<HttpResponseMessage> SendAsync(HttpRequestMessage request, CancellationToken cancellationToken)
        {
            ...
            if (TryRetrieveToken(request, out token))
            {
                ...
                {
                    errorResponse = request.CreateErrorResponse(HttpStatusCode.Unauthorized, "Token Invalid Signature");
                }
                catch (JsonWebToken.TokenValidationException)
                {
                    errorResponse = request.CreateErrorResponse(HttpStatusCode.Unauthorized, "You Must Sign In");
                }
                catch (Exception)
                {
                    errorResponse = request.CreateErrorResponse(HttpStatusCode.InternalServerError, "Token Invalid");
                }
            }
